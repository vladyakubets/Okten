...
for (let j = 0; j < usersWithId.length; j++) {
    for (let k = 0; k < citiesWithId.length; k++) {
        if (usersWithId[j].id === citiesWithId[k].user_id) {
            console.log(i,j,k);
            usersWithCities[i].id = usersWithId[j].id;
            usersWithCities[i].name = usersWithId[j].name;
            usersWithCities[i].age = usersWithId[j].age;
            usersWithCities[i].status = usersWithId[j].status;
            usersWithCities[i].address.user_id = citiesWithId[k].user_id;
            usersWithCities[i].address.country = citiesWithId[k].country;
            usersWithCities[i].address.city = citiesWithId[k].city;
            i++;
        }
    }
}
